...
class AllEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = RestCountriesSDK::test($seed, null);
        $seen = iterator_to_array($base->All(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = RestCountriesConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = RestCountriesSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set REST_COUNTRIES_TEST_ALL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function all_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("REST_COUNTRIES_TEST_ALL_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["REST_COUNTRIES_TEST_ALL_ENTID"]);
