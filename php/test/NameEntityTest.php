...
class NameEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Name(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = name_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "name." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set RESTCOUNTRIES_TEST_NAME_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $name_ref01_data = null;
        if (count($name_ref01_data_raw) > 0) {
        ...
        [$name_ref01_data_dt0_loaded, $err] = $name_ref01_ent->load($name_ref01_match_dt0, null);
        $this->assertNull($err);
...
function name_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/name/NameTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["name01", "name02", "name03"] as $k) {
    ...
    $entid_env_raw = getenv("RESTCOUNTRIES_TEST_NAME_ENTID");
