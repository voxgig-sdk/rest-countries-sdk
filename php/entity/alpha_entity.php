...
// RestCountries SDK Alpha entity
...
class AlphaEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AlphaEntity($this->_client, $opts);
    }

    /**
     * @param Alpha|array $args Alpha data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = RestCountriesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Alpha|array The current Alpha data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Alpha fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = RestCountriesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Alpha fields).
     */
     ...
     * Load a single Alpha.
     *
     * @param AlphaLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AlphaLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Alpha|array The loaded Alpha as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = RestCountriesHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
