...
class CapitalEntity
  ...
  def make
    opts = @_entopts.dup
    CapitalEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RestCountriesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Capital, Hash] the current Capital data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RestCountriesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Capital fields)
  def match_get
  ...
  #
  # @param reqmatch [CapitalLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Capital.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Capital, Hash] the loaded Capital; raises RestCountriesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = RestCountriesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
