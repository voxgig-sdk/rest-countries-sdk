...
class AlphaEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = alpha_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "alpha." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RESTCOUNTRIES_TEST_ALPHA_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    alpha_ref01_data = nil
    if alpha_ref01_data_raw.length > 0
...
def alpha_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "alpha", "AlphaTestData.json")
  ...
  entid_env_raw = ENV["RESTCOUNTRIES_TEST_ALPHA_ENTID"]
