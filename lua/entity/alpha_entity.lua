...
local AlphaEntity = {}
AlphaEntity.__index = AlphaEntity


function AlphaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AlphaEntity)
  self._name = "alpha"
  self._client = client
...
function AlphaEntity:get_name()
  return self._name
end


function AlphaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AlphaEntity.new(self._client, opts)
end
...
function AlphaEntity:mark_deleted()
  self._deleted = true
end


function AlphaEntity:deleted()
  return true == self._deleted
end


function AlphaEntity:data_set(args)
  if args ~= nil then
...
function AlphaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AlphaEntity:match_set(args)
  if args ~= nil then
...
function AlphaEntity:match_get()
...
function AlphaEntity:stream(action, args, callopts)
...
---@param reqmatch AlphaLoadMatch
---@param ctrl? table
---@return Alpha
---@return string? err
function AlphaEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function AlphaEntity:_run_op(ctx, post_done)
  local utility = self._utility
