...
local NameEntity = {}
NameEntity.__index = NameEntity


function NameEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NameEntity)
  self._name = "name"
  self._client = client
...
function NameEntity:get_name()
  return self._name
end


function NameEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NameEntity.new(self._client, opts)
end
...
function NameEntity:mark_deleted()
  self._deleted = true
end


function NameEntity:deleted()
  return true == self._deleted
end


function NameEntity:data_set(args)
  if args ~= nil then
...
function NameEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NameEntity:match_set(args)
  if args ~= nil then
...
function NameEntity:stream(action, args, callopts)
...
---@param reqmatch NameLoadMatch
---@param ctrl? table
---@return Name
---@return string? err
function NameEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function NameEntity:_run_op(ctx, post_done)
  local utility = self._utility
