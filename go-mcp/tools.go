...
type Args struct {
	Entity string         `json:"entity" jsonschema:"all | alpha | capital | name"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.RestCountriesSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "rest-countries_list",
		Description: "List records from RestCountries. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "rest-countries_load",
		Description: "Load a single record from RestCountries. " +
...
func runOp(client *sdk.RestCountriesSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.RestCountriesSDK, name string) (sdk.RestCountriesEntity, error) {
	switch strings.ToLower(name) {
	case "all":
		return client.All(nil), nil
	case "alpha":
		return client.Alpha(nil), nil
	case "capital":
		return client.Capital(nil), nil
	case "name":
