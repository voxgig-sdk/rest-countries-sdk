...
[MCP](https://modelcontextprotocol.io) server exposing the RestCountries SDK as
two agent tools — `rest-countries_list` and `rest-countries_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/rest-countries-mcp)
make build

# 2. Provide credentials via the environment
export REST_COUNTRIES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user rest-countries \
  -- /absolute/path/to/rest-countries-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./rest-countries-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// rest-countries_list: first page of records
{ "entity": "all" }
{ "entity": "all", "query": { } }

// rest-countries_load: one record by id
{ "entity": "alpha", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/rest-countries-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user rest-countries \
     -- "$PWD"/dist/*/rest-countries-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `rest-countries_list` and `rest-countries_load` tools now appear
   in new sessions. Ask the agent to *"list all using rest-countries"*
   and it calls `rest-countries_list` with `{"entity":"all"}`.
...
```sh
export REST_COUNTRIES_APIKEY=sk_live_xxx            # API key
export REST_COUNTRIES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `rest-countries_list` tool
...
### Call the `rest-countries_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `rest-countries_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `rest-countries_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `REST_COUNTRIES_APIKEY` | API key sent with every request. |
| `REST_COUNTRIES_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

all | alpha | capital | name

### Smoke test via HTTP (raw JSON-RPC)

```sh
./rest-countries-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"rest-countries_load","arguments":{"entity":"alpha","query":{"id":1}}}}'
```
