...
class TestCapitalDirect:

    def test_should_direct_load_capital(self):
        setup = _capital_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-capital", "live" if setup["live"] else "unit")
        if _skip:
...
def _capital_direct_setup(mockres):
