...
class RestCountriesTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RestCountriesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RestCountriesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RestCountriesTestRunner.getenv("REST_COUNTRIES_TEST_LIVE")
        override = RestCountriesTestRunner.getenv("REST_COUNTRIES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RestCountriesTestRunner.getenv(key)
        ...
        explain = RestCountriesTestRunner.getenv("REST_COUNTRIES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["REST_COUNTRIES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if RestCountriesTestRunner._test_control is not None:
            return RestCountriesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RestCountriesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RestCountriesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RestCountriesTestRunner.load_test_control()
...
def load_env_local():
    RestCountriesTestRunner.load_env_local()


def env_override(m):
    return RestCountriesTestRunner.env_override(m)


def entity_data(v):
    return RestCountriesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RestCountriesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RestCountriesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RestCountriesTestRunner.load_test_control()


def live_client_options():
    return RestCountriesTestRunner.live_client_options()


def live_delay_ms():
    return RestCountriesTestRunner.live_delay_ms()
