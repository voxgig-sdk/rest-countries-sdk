...
class TestNameDirect:

    def test_should_direct_load_name(self):
        setup = _name_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-name", "live" if setup["live"] else "unit")
        if _skip:
...
def _name_direct_setup(mockres):
