# RestCountries SDK utility: make_request
...
module RestCountriesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RestCountriesResponse.new({})
    ...
    if fetch_err
      ...
      response = RestCountriesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RestCountriesResponse.new(fetched)
    else
