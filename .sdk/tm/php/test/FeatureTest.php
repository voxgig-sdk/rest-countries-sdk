...
// RestCountries SDK feature test
//
...
class FtCtrl extends RestCountriesControl
{
...
class FtClient
{
    ...
    private RestCountriesUtility $utility;

    public function __construct(RestCountriesUtility $utility, array $options)
    {
    ...
    public function get_utility(): RestCountriesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RestCountriesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RestCountriesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RestCountriesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RestCountriesConfig::make_feature($name);
    ...
    private static function buildUrl(RestCountriesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RestCountriesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RestCountriesResponse($fetched);
        }
    ...
    private function fail(RestCountriesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RestCountriesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
