...
// RestCountries SDK retry feature
...
class RestCountriesRetryFeature extends RestCountriesBaseFeature
{
    ...
    public function init(RestCountriesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RestCountriesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RestCountriesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RestCountriesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
