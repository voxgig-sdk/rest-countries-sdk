...
func TestAlphaEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		alphaRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.alpha")))
		...
		alphaRef01MatchDt0 := map[string]any{
			"id": alphaRef01Data["id"],
		}
		alphaRef01DataDt0Loaded, err := alphaRef01Ent.Load(alphaRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		alphaRef01DataDt0LoadResult := core.ToMapAny(entityData(alphaRef01DataDt0Loaded))
		if alphaRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if alphaRef01DataDt0LoadResult["id"] != alphaRef01Data["id"] {
...
func alphaBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("REST_COUNTRIES_TEST_ALPHA_ENTID")
	...
	idmapResolved := core.ToMapAny(env["REST_COUNTRIES_TEST_ALPHA_ENTID"])
