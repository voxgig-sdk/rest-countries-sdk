...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("REST_COUNTRIES_TEST_LIVE") == "TRUE" ||
		os.Getenv("REST_COUNTRIES_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("REST_COUNTRIES_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.RestCountriesSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.RestCountriesError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.RestCountriesSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.RestCountriesError{Msg: msg, Code: code}
}
