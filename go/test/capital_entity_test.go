...
func TestCapitalEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		capitalRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.capital")))
...
func capitalBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("REST_COUNTRIES_TEST_CAPITAL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["REST_COUNTRIES_TEST_CAPITAL_ENTID"])
