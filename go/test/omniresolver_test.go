...
import (
	...
	omni "github.com/voxgig-sdk/rest-countries-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RestCountriesError); is {
				out := map[string]any{
					"name":    "RestCountriesError",
...
func omniCtx(arg any, client *sdk.RestCountriesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RestCountriesSDK); is {
			client = live
