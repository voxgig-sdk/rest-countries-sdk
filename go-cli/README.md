...
boru-driven command-line client **and** interactive REPL for the RestCountries
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/rest-countries-cli)
make build

# 2. See usage (words, entities, env vars)
./rest-countries-cli --help

# 3. Provide credentials once, via the environment
export REST_COUNTRIES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./rest-countries-cli list all
./rest-countries-cli list alpha

# 5. Override the API base URL for a single call
REST_COUNTRIES_BASE=https://api.example.com ./rest-countries-cli list all

# 6. No arguments -> interactive REPL
./rest-countries-cli
rest-countries> list all
rest-countries> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/rest-countries-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/rest-countries-cli list all
   ```
...
Run with no arguments to open a REPL (prompt `rest-countries>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/rest-countries-cli`. |
...
(`./rest-countries-cli <expr>`) and interactively (the REPL), and expressions compose the
