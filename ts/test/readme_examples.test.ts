...
//     (\`new RestCountriesSDK(...)\` / \`RestCountriesSDK.test(...)\` become
//     \`RestCountriesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RestCountriesSDK'
...
const TEST_SEED = {"entity":{"all":{"test01":{"id":"test01"}},"alpha":{"test01":{"id":"test01"}},"capital":{"test01":{"id":"test01"}},"name":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RestCountriesSDK(...)\` and any
// \`RestCountriesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RestCountriesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RestCountriesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
